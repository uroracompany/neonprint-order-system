-- Notification improvements for Quote and Production flows.
-- 1. send_order_to_production: adds order ID to messages, sends confirmation to actor
-- 2. handle_order_change_notification: notifies quote user on seller edits,
--    includes assigned production users in global status changes
