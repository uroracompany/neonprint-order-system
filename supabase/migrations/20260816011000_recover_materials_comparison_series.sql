-- Recuperación idempotente del RPC de comparación de Materiales.
-- Reemplaza la versión inicial que no alcanzó a entrar en la caché de PostgREST
-- e incorpora la granularidad exacta para no sumar variedades diarias.

DROP FUNCTION IF EXISTS public.kpi_materials_comparison_series(
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object('value', id::text, 'label', name) ORDER BY name), '[]'::jsonb)
  INTO v_materials
  FROM catalog;
  ...
  SELECT jsonb_build_object(
    'metrics', coalesce((
      ...
      FROM scope_metrics WHERE scope = 'period'
    ), jsonb_build_object(
  ...
  SELECT jsonb_build_object(
    'metrics', coalesce((
      ...
      FROM scope_metrics WHERE scope = 'comparison'
    ), jsonb_build_object(
      'references', 0, 'orders_with_material', 0, 'materials_used', 0,
      'normal_orders', 0, 'urgent_orders', 0, 'internal_orders', 0, 'external_orders', 0
    )),
