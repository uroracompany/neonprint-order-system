-- Suppress duplicate status-change notifications when Seller assigns an order
-- to Design. The assignment confirmation remains the user-facing notification.
...
where coalesce(n.metadata->>'event_kind', '') = 'status_changed'
  and coalesce(n.metadata->>'status', '') in ('in_Design', 'In_Design')
  and n.order_id is not null
  and exists (
    select 1
    from public.notifications c
    where c.user_id = n.user_id
      and c.order_id is not distinct from n.order_id
      and c.id <> n.id
      and coalesce(c.metadata->>'event_kind', '') = 'designer_assigned_confirmation'
      and (c.metadata->>'actor_id') is not distinct from (n.metadata->>'actor_id')
      and c.created_at between n.created_at - interval '2 minutes'
                           and n.created_at + interval '2 minutes'
  );
