-- Corrective hardening for role-scoped order assignments and notifications.
-- Prevents seller accounts from receiving notifications intended only for
-- designers/quote users when assignment ids are mis-targeted or stale.
...
begin
  if new.designer_id is not null and not exists (
    ...
    raise exception 'designer_id must reference an active designer profile';
  end if;
  ...
  if new.quote_id is not null and not exists (
    ...
    raise exception 'quote_id must reference an active quote profile';
  end if;
...
revoke all on function public.validate_order_assignment_roles() from public;
revoke all on function public.validate_order_assignment_roles() from anon;
revoke all on function public.validate_order_assignment_roles() from authenticated;

drop trigger if exists trg_validate_order_assignment_roles on public.orders;
create trigger trg_validate_order_assignment_roles
  before insert or update of designer_id, quote_id on public.orders
  for each row
  execute function public.validate_order_assignment_roles();
...
where
  (
    coalesce(n.metadata->>'event_kind', '') = 'designer_assigned'
    and not exists (
      select 1
      from public.profiles p
  ...
  or (
    coalesce(n.metadata->>'event_kind', '') = 'quote_assigned'
    and not exists (
      select 1
      from public.profiles p
      where p.id = n.user_id
  ...
  or (
    coalesce(n.metadata->>'event_kind', '') = 'quote_sent_confirmation'
    and not exists (
      select 1
      from public.profiles p
      where p.id = n.user_id
  ...
  or (
    ...
    and coalesce(n.metadata->>'previous_status', '') in ('cotizacion', 'in_Quotation')
