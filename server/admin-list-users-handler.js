...
export async function handleAdminListUsers(payload = {}, env = process.env) {
  ...
  let query = supabaseAdmin
    .from("profiles")
    .select("id,name,email,role,employment_status,deleted_at,deleted_by,deletion_reason,created_at", { count: "exact" });
  ...
  if (isMissingEmailColumnError(error)) {
    let fallbackQuery = supabaseAdmin
      .from("profiles")
      .select("id,name,role,employment_status,deleted_at,deleted_by,deletion_reason,created_at", { count: "exact" });
