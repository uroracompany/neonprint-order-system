...
import { formatNumber, getComparePeriodBounds, getPeriodBounds } from '../../utils/kpiHelpers'
import { Icons } from '../../utils/icons'
import { COMPARISON_COLORS } from '../../utils/materialsComparisonColors'
...
const ALL = '__all__'
const SPLIT_COLORS = Object.freeze({ blue: '#2454D9', pink: '#F43F5E' })
...
const COMPARISON_PERIOD_OPTIONS = [
  { value: 'month', label: 'Este mes vs. mes anterior' },
  { value: '2months', label: 'Últimos 2 meses vs. los 2 anteriores' },
  { value: '3months', label: 'Últimos 3 meses vs. los 3 anteriores' },
  { value: 'week', label: 'Esta semana vs. semana anterior' },
]
...
function getPresetBounds(preset) {
  const current = getPeriodBounds(preset)
  const comparison = getComparePeriodBounds(preset, current)
  return {
...
function isOpenRange(bounds) {
  const now = new Date()
  return now >= new Date(bounds.date_from) && now < new Date(bounds.date_to)
}
...
function SplitMetric({ label, currentFirst, currentSecond, previousFirst, previousSecond, firstLabel, secondLabel, firstColor, secondColor, loading }) {
  ...
  return <article className="kpi-materials-compare-split"><div><span>{label}</span><small>{currentPercent}% {firstLabel.toLowerCase()} · antes {previousPercent}%</small></div><div className={`kpi-materials-compare-split-bar${hasCurrentData ? '' : ' is-empty'}`} aria-label={`${label}: ${firstLabel} ${currentFirst}, ${secondLabel} ${currentSecond}`}>{hasCurrentData && <><i className="kpi-materials-compare-split-segment" style={{ width: `${currentPercent}%`, '--split-color': firstColor }} /><i className="kpi-materials-compare-split-segment" style={{ width: `${100 - currentPercent}%`, '--split-color': secondColor }} /></>}</div><div className="kpi-materials-compare-split-legend"><span><i className="kpi-materials-compare-split-dot" style={{ '--split-color': firstColor }} />{firstLabel} <strong>{formatNumber(currentFirst)}</strong></span><span><i className="kpi-materials-compare-split-dot" style={{ '--split-color': secondColor }} />{secondLabel} <strong>{formatNumber(currentSecond)}</strong></span></div></article>
}

function ComparisonChartTooltip({ active, payload, label }) {
  if (!active || !payload?.length) return null
  const point = payload[0]?.payload
  const hasPeakReference = Number(point?.comparisonReference) > 0
  return <div className="kpi-materials-compare-tooltip"><strong>{label}</strong><span className="is-current" style={{ '--comparison-current-color': point?.comparisonColor || COMPARISON_COLORS.flat }}><i />Período actual ({point?.periodALabel}): <b>{point?.periodA === null ? 'Sin dato aún' : formatNumber(point?.periodA || 0)}</b></span><span className="is-reference"><i />Máximo del período anterior: <b>{hasPeakReference ? formatNumber(point.comparisonReference) : 'Sin referencia comparable'}</b></span><span className="is-previous"><i />Período anterior ({point?.periodBLabel}): <b>{formatNumber(point?.periodB || 0)}</b></span></div>
}
...
export default function MaterialsComparisonPanel({ userId }) {
  ...
  const requestBounds = useMemo(() => getPresetBounds(preset), [preset])
  ...
  const materialOptions = useMemo(() => {
    const source = analytics?.materials || []
  ...
  const chartData = useMemo(() => analytics ? buildMaterialComparisonChartRows(analytics.period.timeline, analytics.comparison.timeline, requestBounds, selectedMetric.key, granularity) : null, [analytics, granularity, requestBounds, selectedMetric.key])
  const hasChartData = chartData?.rows.some(row => row.periodA > 0 || row.periodB > 0)
  const comparisonShares = useMemo(() => analytics
    ? getMaterialComparisonShares(currentMetrics?.[selectedMetric.key], previousMetrics?.[selectedMetric.key])
    : null, [analytics, currentMetrics, previousMetrics, selectedMetric.key])
  const hasOrderFilter = orderType !== ALL || designType !== ALL
  const currentRangeIsOpen = isOpenRange(requestBounds)
  ...
  return (
    <section className="kpi-section" aria-label="Comparación de rendimiento de materiales">
      <div className="kpi-section-header"><div><span className="kpi-section-kicker">Comparación</span><h2 className="kpi-section-title">Rendimiento de materiales por período</h2><p className="kpi-section-subtitle">Compara totales de referencias, órdenes con material, variedad y composición frente al período anterior de igual duración.</p></div></div>
      <div className="kpi-card kpi-materials-comparison-workspace" data-comparison-state={hasFailure ? 'error' : isLoading ? 'loading' : analytics ? 'ready' : 'idle'}>
        <div className="kpi-materials-compare-controls">
          <FilterSelect label="Base temporal" icon={<Icons.Calendar />} value={preset} onChange={setPreset} options={COMPARISON_PERIOD_OPTIONS} />
