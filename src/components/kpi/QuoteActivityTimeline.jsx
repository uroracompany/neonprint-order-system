...
const EVENT_CONFIG = {
  ...
  order_updated:                { icon: Icons.Edit,       color: '#0EA5E9', label: 'Editó la orden' },
  ...
  admin_edited_order:           { icon: Icons.Edit,       color: '#0EA5E9', label: 'Editó la orden' },
  client_created:               { icon: Icons.Users,      color: '#0EA5E9', label: 'Registró cliente' },
  default:                      { icon: Icons.Clipboard,  color: '#94A3B8', label: 'Acción' },
}
...
function getEventDescription(event) {
  ...
  if (event.type === 'admin_intervention') {
    ...
    const actionLabels = {
      route_quote: 'Recibió orden en caja',
      set_quote_assignee: 'Reasignado como cotizador',
...
export default function QuoteActivityTimeline({ quoteId, getDateBounds }) {
  ...
  }, [quoteId, getDateBounds, offset])

  useEffect(() => { fetchEvents() }, [quoteId, getDateBounds]) // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className="kpi-card" style={{ padding: 24, marginBottom: 24 }}>
      ...
      {total > 0 && (
        <div className="kpi-seller-timeline-filters">
          ...
          />
          <div className="kpi-seller-timeline-search" hidden>
        ...
        <div className="kpi-empty-state" style={{ padding: 30 }}>
          ...
          <div className="kpi-empty-message">
            {hasFilters
              ...
              : 'No hay eventos registrados para este cotizador en el período seleccionado.'}
          </div>
        </div>
      ) : (
