...
export function FlowTrackClient({ status, events, order, designType }) {
  ...
  const getStepState = (step, i) => {
    ...
    return { isCompleted, isActive, isQuoteBlocked };
  };
    ...
    <div className="ftc-wrap">
      ...
      {!isCancelled && currentStepIdx >= 0 && (
        <div className="ftc-timeline-horizontal">
          {steps.map((step, i) => {
            const { isCompleted, isActive, isQuoteBlocked } = getStepState(step, i);
            ...
            return (
              <div key={step.key} className="ftc-h-step-wrap">
                <div className={`ftc-h-step ${isCompleted ? "completed" : ""} ${isActive ? "active" : ""} ${isQuoteBlocked ? "blocked" : ""}`}>
                  <div className={`ftc-h-circle ${isCompleted ? "done" : isQuoteBlocked ? "blocked" : isActive ? "active" : ""}`}>
                    {isCompleted && (
                ...
                {i < steps.length - 1 && (
                  <div className={`ftc-h-line ${isCompleted ? "done" : isQuoteBlocked ? "pending-payment" : ""}`} />
                )}
              </div>
            );
          })}
            ...
            return (
              <div key={step.key} className={`ftc-v-step ${isCompleted ? "completed" : ""} ${isActive ? "active" : ""} ${isQuoteBlocked ? "blocked" : ""}`}>
                <div className="ftc-v-indicator">
                  <div className={`ftc-v-dot ${isCompleted ? "done" : isQuoteBlocked ? "blocked" : isActive ? "active" : ""}`}>
                  ...
                  {i < steps.length - 1 && (
                    <div className={`ftc-v-line ${isCompleted ? "done" : isQuoteBlocked ? "pending-payment" : ""}`} />
                  )}
                </div>
